...
bool FillMemoryOnAllocation = true;
#endif
...
static char* AllocWithMMap(uintptr_t sz, EMMapMode mode)
{
    ...
    if (mode == MM_HUGE) {
        largeBlock = AllocWithMMapLinuxImpl(
            sz,
            reinterpret_cast<char* volatile*>(&linuxAllocPointerHuge),
            reinterpret_cast<char*>(LINUX_MMAP_AREA_START + N_MAX_WORKSET_SIZE),
            N_HUGE_AREA_FINISH);
    } else {
        largeBlock = AllocWithMMapLinuxImpl(
            sz,
            reinterpret_cast<char* volatile*>(&linuxAllocPointer),
            reinterpret_cast<char*>(LINUX_MMAP_AREA_START),
            N_MAX_WORKSET_SIZE);
        if (Y_UNLIKELY(uintptr_t(((char*)largeBlock - ALLOC_START) + sz) >= N_MAX_WORKSET_SIZE))
            NMalloc::AbortFromCorruptedAllocator(); // out of working set, something has broken
    }
